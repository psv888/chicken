          ...
          <div key={s.key} className="admin-dashboard-section">
            <div className="admin-section-header" style={{display: 'flex', alignItems: 'center', gap: 18, marginBottom: 28}}>
              ...
              <input
                ...
                style={{flex: 1, maxWidth: 220, padding: '7px 12px', borderRadius: 8, border: '1px solid #eee', fontSize: '1rem'}}
              />
            </div>
                  ...
                  >
                    {editingParentId === item.id ? (
                      <form className="admin-dashboard-edit-form" onSubmit={e => handleEditParentSubmit(e, item)}>
                        ...
                        <button type="button" onClick={cancelEditParent}>Cancel</button>
                      </form>
                    ) : (
                      <>
                        ...
                        <div className="admin-dashboard-item-actions">
                          <button onClick={() => startEditParent(item)}>Edit</button>
                          <button onClick={() => handleDeleteParent(item)}>Delete</button>
                        </div>
                        {/* Menu Card */}
                        <div className="admin-menu-card" onClick={() => { setMenuModalParent(item); setSelectedParent(item.id); }} tabIndex={0} role="button" aria-label="Menu" style={{marginTop: 16, cursor: 'pointer'}}>
                    ...
                    {/* Dishes for this parent */}
                    {dishes[item.id] && dishes[item.id].length > 0 && (
                      <div className="admin-dashboard-dishes-list">
                        ...
                        <input
                          ...
                          value={dishSearch[item.id] || ''}
                          onChange={e => setDishSearch(prev => ({ ...prev, [item.id]: e.target.value }))}
                        ...
                        <div className="admin-dashboard-dishes-cards">
                          {dishes[item.id]
                            .filter(dish =>
                              !dishSearch[item.id] || dish.name.toLowerCase().includes(dishSearch[item.id].toLowerCase())
                            )
                            .map((dish, dIdx) => (
                              <div key={dIdx} className="admin-dashboard-dish-card">
                                {editingDish.parentId === item.id && editingDish.idx === dIdx ? (
                                  <form className="admin-dashboard-edit-form" onSubmit={e => handleEditDishSubmit(e, item.id, dIdx)}>
                                    <input
                                      type="file"
                                    ...
                                    />
                                    <button type="submit">Save</button>
                                    <button type="button" onClick={cancelEditDish}>Cancel</button>
                                  </form>
                                ) : (
                                  <>
                                    ...
                                    <div className="admin-dashboard-item-actions">
                                      <button onClick={() => startEditDish(item.id, dIdx, dish)}>Edit</button>
                                      <button onClick={() => handleDeleteDish(item.id, dIdx)}>Delete</button>
                                    </div>
                                  </>
                                )}
                              </div>
          ...
          <>
            ...
            <div className="admin-dashboard-dishes-list">
              ...
              <input
                ...
                type="text"
                placeholder="Search dishes..."
                value={dishSearch[menuModalParent.id] || ''}
                onChange={e => setDishSearch(prev => ({ ...prev, [menuModalParent.id]: e.target.value }))}
              />
              <div className="admin-dashboard-dishes-cards">
                {dishes[menuModalParent.id]
                  .filter(dish =>
                    !dishSearch[menuModalParent.id] || dish.name.toLowerCase().includes(dishSearch[menuModalParent.id].toLowerCase())
                  )
                  .map((dish, dIdx) => (
                    <div key={dIdx} className="admin-dashboard-dish-card">
                      {editingDish.parentId === menuModalParent.id && editingDish.idx === dIdx ? (
                        <form className="admin-dashboard-edit-form" onSubmit={e => handleEditDishSubmit(e, menuModalParent.id, dIdx)}>
                          <input
                            type="file"
                            ref={editDishPhotoInput}
                            name="photo"
                        ...
                        <>
                          ...
                          <div className="admin-dashboard-dish-price">₹{dish.price}</div>
                          <div className="admin-dashboard-item-actions">
                            <button onClick={() => startEditDish(menuModalParent.id, dIdx, dish)}>Edit</button>
                            <button onClick={() => handleDeleteDish(menuModalParent.id, dIdx)}>Delete</button>
                          </div>
                        </>
                      )}
                    </div>
